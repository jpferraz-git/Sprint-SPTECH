...
const serial = async (
    ...
    if (SIMULAR_ARDUINO) {
        ...
        setInterval(() => {
            ...
            // Insere no banco se habilitado, para habilitar coloque (TRUE) na 'HABILITAR_OPERACAO_INSERIR'
            if (HABILITAR_OPERACAO_INSERIR) {
                poolBancoDados.execute(
                    'INSERT INTO medida (sensor_analogico) VALUES (?)',
                    [sensorAnalogico]
                ).then(() => {
                    console.log("Valores simulados inseridos no banco");
                }).catch((erro) => {
                    console.error("Erro ao inserir valores simulados no banco: ", erro);
                });
            }
        }, 2000);  // Simula dados a cada 1 segundo
    } else {
